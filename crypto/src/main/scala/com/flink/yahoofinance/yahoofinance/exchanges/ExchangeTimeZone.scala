package com.flink.yahoofinance.yahoofinance.exchanges
...
object ExchangeTimeZone extends Logging {

  val SUFFIX_TIMEZONES = new util.HashMap[String, TimeZone]
  val INDEX_TIMEZONES = new util.HashMap[String, TimeZone]
   ...
   */
  def get(suffix: String): TimeZone = {
    if (SUFFIX_TIMEZONES.containsKey(suffix)) return SUFFIX_TIMEZONES.get(suffix)
    log.warn("Cannot find time zone for exchange suffix: '{}'. Using default: America/New_York", suffix)
    SUFFIX_TIMEZONES.get("")
  }
   ...
   */
  def getStockTimeZone(symbol: String): TimeZone = { // First check if it's a known stock index
    if (INDEX_TIMEZONES.containsKey(symbol)) return INDEX_TIMEZONES.get(symbol)
    if (!symbol.contains(".")) return ExchangeTimeZone.get("")
    val split = symbol.split("\\.")
    ExchangeTimeZone.get(split(split.length - 1))
  }
